...
void chain_set_rgb_value(uint8_t *buffer, uint8_t size)
{
    ...
    if (buffer != NULL && size == 5 && buffer[0] == 0 && buffer[1] == (RGB_NUM - 1)) {
...
void chain_get_rgb_value(uint8_t *buffer, uint8_t size)
{
    ...
    // Validate input and check buffer format
    if (buffer != NULL && size == 2 && buffer[0] == 0 && buffer[1] == (RGB_NUM - 1)) {
        operation_result = OPERATION_SUCCESS;
    }
...
void rgb_update(void)
{
    ...
    if (fade_frames_left > 0 && (now - last_step_tick) >= RGB_FADE_MS_STEP) {
        ...
        if (fade_type == 1) {  // R
            ...
            event_overlay.B = (uint8_t)(((fade_frames_left - 1) * RGB_FADE_MAX) / (RGB_FADE_FRAMES - 1));
            event_overlay.R = 0;
            event_overlay.G = 0;
        }
        fade_frames_left--;
