...
int main(void)
{
    ...
    while (1) {
        if (g_cmd_status == CMD_SPACE_BUSY_STATUS) {
            switch (g_cmd_buf[0]) {
                case CHAIN_I2C_INIT:
                    chain_i2c_init(g_cmd_buf[1]);
                    break;
                case CHAIN_I2C_READ:
                    chain_i2c_read(g_cmd_buf[1], g_cmd_buf[2]);
                    break;
                case CHAIN_I2C_WRITE:
                    chain_i2c_write(g_cmd_buf[1], (uint8_t *)(g_cmd_buf + 3), g_cmd_buf[2]);
                    break;
                case CHAIN_I2C_MEM_READ:
                    chain_i2c_mem_read(g_cmd_buf[1], ((g_cmd_buf[4] << 8) | g_cmd_buf[3]), g_cmd_buf[2], g_cmd_buf[5]);
                    break;
                case CHAIN_I2C_MEM_WRITE:
                    chain_i2c_mem_write(g_cmd_buf[1], ((g_cmd_buf[4] << 8) | g_cmd_buf[3]), g_cmd_buf[2], g_cmd_buf[5],
                                        (uint8_t *)(g_cmd_buf + 6));
                    break;
                case CHAIN_I2C_SCAN_ADDR:
                    chain_i2c_scan_addr();
                    break;
                case CHAIN_GPIO_OUTPUT_INIT:
                    chain_uart_output_init((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_GPIO_SET_OUTPUT_LEVEL:
                    chain_uart_set_output_level(g_cmd_buf[1], g_cmd_buf[2]);
                    break;
                case CHAIN_GPIO_GET_OUTPUT_LEVEL:
                    chain_uart_get_output_level(g_cmd_buf[1]);
                    break;
                case CHAIN_GPIO_INPUT_INIT:
                    chain_uart_input_init((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_GPIO_READ_GPIO_LEVEL:
                    chain_uart_read_pin_level(g_cmd_buf[1]);
                    break;
                case CHAIN_GPIO_EXTERNAL_NVIC_INIT:
                    chain_uart_nvic_init((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_GPIO_ADC_INIT:
                    chain_uart_adc_init(g_cmd_buf[1]);
                    break;
                case CHAIN_GPIO_ADC_READ:
                    chain_uart_adc_read(g_cmd_buf[1]);
                    break;
                case CHAIN_GET_WORK_STATION:
