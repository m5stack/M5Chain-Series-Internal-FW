...
void DMA1_Channel2_3_IRQHandler(void)
{
    ...
    if (LL_DMA_IsActiveFlag_TC2(DMA1)) {
        ...
        if (tx_out_buffer.packet_count > 0) {
            ...
            LL_DMA_SetMemoryAddress(DMA1, LL_DMA_CHANNEL_2, (uint32_t)tx_out_buffer.send_queue[index].data);
...
void DMA1_Ch4_5_DMAMUX1_OVR_IRQHandler(void)
{
    ...
    if (LL_DMA_IsActiveFlag_TC4(DMA1)) {
        ...
        if (tx_in_buffer.packet_count > 0) {
            ...
            LL_DMA_SetMemoryAddress(DMA1, LL_DMA_CHANNEL_4, (uint32_t)tx_in_buffer.send_queue[index].data);
...
void TIM14_IRQHandler(void)
{
    ...
    if (LL_TIM_IsActiveFlag_UPDATE(TIM14) == SET) {
        ...
        usart2_transmit_dma(s_enum_please_data_packet_buf, sizeof(s_enum_please_data_packet_buf));
...
void TIM17_IRQHandler(void)
{
    ...
    if (LL_TIM_IsActiveFlag_UPDATE(TIM17) == SET) {
        ...
        usart1_transmit_dma(g_heartbeat_data_pack_buf, sizeof(g_heartbeat_data_pack_buf));
    }
...
void USART1_IRQHandler(void)
{
    ...
    if (LL_USART_IsActiveFlag_IDLE(USART1) && LL_USART_IsEnabledIT_IDLE(USART1)) {
        ...
        // Temporarily disable DMA channel for USART1 RX
        ...
        LL_DMA_SetMemoryAddress(DMA1, LL_DMA_CHANNEL_1, (uint32_t)g_uart_out_rx_buf[g_uart_out_rx_index]);
        LL_DMA_SetDataLength(DMA1, LL_DMA_CHANNEL_1, BUFFER_SIZE);
        LL_DMA_EnableChannel(DMA1, LL_DMA_CHANNEL_1);  // Re-enable DMA

        // Re-enable USART1 DMA request for RX
        LL_USART_EnableDMAReq_RX(USART1);

        // Process received data if length is valid
        ...
        while (p <= end - MIN_PACKET_SIZE) {
            ...
            p += total_length;  // Move to next packet
        }
    }
...
void USART2_IRQHandler(void)
{
    ...
    if (LL_USART_IsActiveFlag_IDLE(USART2) && LL_USART_IsEnabledIT_IDLE(USART2)) {
        ...
        // Set new DMA receive buffer address and size
        LL_DMA_SetMemoryAddress(DMA1, LL_DMA_CHANNEL_3, (uint32_t)g_uart_in_rx_buf[g_uart_in_rx_index]);
