...
/**
 * @brief Set the threshold value in flash memory.
 * @note This function reads the current threshold value, modifies it with the
 *       new data, erases the flash page, and writes the updated value back to flash.
 *
 * @param data The new threshold value to set (0-65535).
 * @retval true if the threshold value is successfully set, false otherwise.
 */
bool set_threshold_value(uint16_t data)
{
    __disable_irq();
    uint64_t temp = my_flash_read_double_word(STM32G0xx_FLASH_PAGE30_STARTADDR);  // Read current threshold value
    set_byte_in_uint64(&temp, 0, data & 0xFF);                                    // Set the lower byte of the threshold
    set_byte_in_uint64(&temp, 1, (data >> 8) & 0xFF);                             // Set the upper byte of the threshold
    ...
    __enable_irq();
    // Verify if the new threshold value has been successfully set
    return (get_threshold_value() == data);
}

/**
 * @brief Retrieve the threshold value from flash memory.
 * @note This function reads the threshold value stored at the defined address
 *       and returns it as a 16-bit unsigned integer.
 *
 * @param None
 * @retval The current threshold value.
 */
uint16_t get_threshold_value(void)
{
    return *((__IO uint16_t *)(THRESHOLD_ADDR));  // Read and return the threshold value from flash memory
}
