...
void USART1_IRQHandler(void)
{
    ...
    if (LL_USART_IsActiveFlag_IDLE(USART1) && LL_USART_IsEnabledIT_IDLE(USART1)) {
        ...
        const volatile uint8_t *incoming_buffer = g_uart_in_rx_buf[current_index];
        const volatile uint8_t *end             = incoming_buffer + s_len;
        const volatile uint8_t *p               = incoming_buffer;
        ...
        while (p <= end - MIN_PACKET_SIZE) {
            ...
            switch (packet_type) {
                case CHAIN_ENUM:
                    chain_enum_handle((uint8_t *)p, total_length);
                    break;
                ...
                default:
                    if (p[4] != 1) {
                        chain_out_relay_handle((uint8_t *)p, total_length);
                    } else if (pack_check((uint8_t *)p, total_length) == CHECK_PASS) {
                        chain_deal_data_packet_handle((uint8_t *)p, total_length);
                    }
                    break;
            }
...
void USART2_IRQHandler(void)
{
    ...
    if (LL_USART_IsActiveFlag_IDLE(USART2) && LL_USART_IsEnabledIT_IDLE(USART2)) {
        ...
        const volatile uint8_t *incoming_buffer = g_uart_out_rx_buf[current_index];
        const volatile uint8_t *end             = incoming_buffer + s_len;
        const volatile uint8_t *p               = incoming_buffer;
        ...
        while (p <= end - MIN_PACKET_SIZE) {
            ...
            switch (packet_type) {
                ...
                case CHAIN_ENUM:
                    chain_enum_return_handle((uint8_t *)p, total_length);
                    break;

                case CHAIN_ENUM_PLEASE:
                    chain_enum_please_handle((uint8_t *)p, total_length);
                    break;

                default:
                    // Handle other packets (relay data)
                    chain_in_relay_handle((uint8_t *)p, total_length);
                    break;
            }
