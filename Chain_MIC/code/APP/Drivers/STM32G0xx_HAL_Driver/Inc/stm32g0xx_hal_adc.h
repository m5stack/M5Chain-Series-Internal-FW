...
typedef struct {
    ...
    FunctionalState
        LowPowerAutoWait; /*!< Select the dynamic low power Auto Delay: new conversion start only when the
    ...
    FunctionalState DiscontinuousConvMode; /*!< Specify whether the conversions sequence of ADC group regular is
                                              ...
                                              sequencer is enabled (parameter 'ScanConvMode'). If sequencer is disabled,
                                              this parameter is discarded. Discontinuous mode can be enabled only if
                                              continuous mode is disabled. If continuous mode is enabled, this parameter
                                              setting is discarded. This parameter can be set to ENABLE or DISABLE.
    ...
    uint32_t ExternalTrigConvEdge; /*!< Select the external event edge used to trigger ADC group regular conversion
                                      start If trigger source is set to ADC_SOFTWARE_START, this parameter is discarded.
...
#define HAL_ADC_STATE_INJ_BUSY                                                      \
    (0x00001000UL) /*!< Not available on this STM32 series: A conversion on group   \
...
#define HAL_ADC_STATE_INJ_EOC                                               \
    (0x00002000UL) /*!< Not available on this STM32 series: Conversion data \
                        available on group injected */
#define HAL_ADC_STATE_INJ_JQOVF                                                     \
    (0x00004000UL) /*!< Not available on this STM32 series: Injected queue overflow \
                        occurrence */
...
#define HAL_ADC_STATE_MULTIMODE_SLAVE                                              \
    (0x00100000UL) /*!< Not available on this STM32 series: ADC in multimode slave \
...
#define ADC_SCAN_DISABLE                                     \
    (0x00000000UL) /*!< Sequencer set to fully configurable: \
only the rank 1 is enabled (no scan sequence on several ranks) */
#define ADC_SCAN_ENABLE                                             \
    (ADC_CFGR1_CHSELRMOD) /*!< Sequencer set to fully configurable: \
sequencer length and each rank affectation to a channel are configurable. */

/* Sequencer set to not fully configurable */
#define ADC_SCAN_SEQ_FIXED                                                    \
    (ADC_SCAN_SEQ_FIXED_INT) /*!< Sequencer set to not fully configurable:    \
...
#define ADC_SCAN_SEQ_FIXED_BACKWARD                                                              \
    (ADC_SCAN_SEQ_FIXED_INT | ADC_CFGR1_SCANDIR) /*!< Sequencer set to not fully configurable:   \
                   sequencer length and each rank affectation to a channel are fixed by          \
                   channel HW number (channel 0 fixed on rank 0, channel 1 fixed on rank1, ...). \
...
#define ADC_EXTERNALTRIG_T1_CC4                                      \
    (LL_ADC_REG_TRIG_EXT_TIM1_CH4) /*!< ADC group regular conversion \
...
#define ADC_RANK_CHANNEL_NUMBER                                            \
    (0x00000001U) /*!< Enable the rank of the selected channels. Number of \
...
#define ADC_RANK_NONE                                                      \
    (0x00000002U) /*!< Disable the selected rank (selected channel) from   \
sequencer.                                                                 \
Setting relevant if parameter "ScanConvMode" is set to sequencer not fully \
configurable. */
...
#define ADC_SAMPLINGTIME_COMMON_1                                              \
    (LL_ADC_SAMPLINGTIME_COMMON_1) /*!< Set sampling time common to a group of \
                                        channels: sampling time nb 1 */
#define ADC_SAMPLINGTIME_COMMON_2                                              \
    (LL_ADC_SAMPLINGTIME_COMMON_2) /*!< Set sampling time common to a group of \
...
#define ADC_ANALOGWATCHDOG_SINGLE_REG                                        \
    (ADC_CFGR1_AWD1SGL | ADC_CFGR1_AWD1EN) /*!< ADC AWD applied to a regular \
    group single channel */
#define ADC_ANALOGWATCHDOG_ALL_REG                     \
    (ADC_CFGR1_AWD1EN) /*!< ADC AWD applied to regular \
group all channels */
/**
 * @}
 */
 ...
 */
#define ADC_IS_SOFTWARE_START_REGULAR(__HANDLE__) (((__HANDLE__)->Instance->CFGR1 & ADC_CFGR1_EXTEN) == 0UL)
 ...
 * @retval None
 */
#define ADC_CFGR1_REG_DISCCONTINUOUS(_REG_DISCONTINUOUS_MODE_) ((_REG_DISCONTINUOUS_MODE_) << 16U)
...
#define ADC_SCAN_SEQ_MODE(_SCAN_MODE_)                                                               \
    ((((_SCAN_MODE_) & ADC_SCAN_SEQ_FIXED_INT) != 0UL) ? ((_SCAN_MODE_) & (~ADC_SCAN_SEQ_FIXED_INT)) \
                                                       : (ADC_CFGR1_CHSELRMOD))
...
#define IS_ADC_CLOCKPRESCALER(ADC_CLOCK)                                                       \
    (((ADC_CLOCK) == ADC_CLOCK_SYNC_PCLK_DIV1) || ((ADC_CLOCK) == ADC_CLOCK_SYNC_PCLK_DIV2) || \
     ((ADC_CLOCK) == ADC_CLOCK_SYNC_PCLK_DIV4) || ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV1) ||     \
     ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV2) || ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV4) ||         \
     ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV6) || ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV8) ||         \
     ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV10) || ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV12) ||       \
     ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV16) || ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV32) ||       \
     ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV64) || ((ADC_CLOCK) == ADC_CLOCK_ASYNC_DIV128) ||      \
...
#define IS_ADC_SCAN_MODE(SCAN_MODE)                                                                                  \
    (((SCAN_MODE) == ADC_SCAN_DISABLE) || ((SCAN_MODE) == ADC_SCAN_ENABLE) || ((SCAN_MODE) == ADC_SCAN_SEQ_FIXED) || \
...
#define IS_ADC_EXTTRIG(REGTRIG)                                                             \
    (((REGTRIG) == ADC_EXTERNALTRIG_T1_TRGO2) || ((REGTRIG) == ADC_EXTERNALTRIG_T1_CC4) ||  \
     ((REGTRIG) == ADC_EXTERNALTRIG_T2_TRGO) || ((REGTRIG) == ADC_EXTERNALTRIG_T3_TRGO) ||  \
     ((REGTRIG) == ADC_EXTERNALTRIG_T6_TRGO) || ((REGTRIG) == ADC_EXTERNALTRIG_T15_TRGO) || \
     ((REGTRIG) == ADC_EXTERNALTRIG_EXT_IT11) || ((REGTRIG) == ADC_SOFTWARE_START))
#elif defined(TIM15) && defined(TIM6)
#define IS_ADC_EXTTRIG(REGTRIG)                                                              \
    (((REGTRIG) == ADC_EXTERNALTRIG_T1_TRGO2) || ((REGTRIG) == ADC_EXTERNALTRIG_T1_CC4) ||   \
     ((REGTRIG) == ADC_EXTERNALTRIG_T3_TRGO) || ((REGTRIG) == ADC_EXTERNALTRIG_T6_TRGO) ||   \
     ((REGTRIG) == ADC_EXTERNALTRIG_T15_TRGO) || ((REGTRIG) == ADC_EXTERNALTRIG_EXT_IT11) || \
...
#define IS_ADC_EXTTRIG(REGTRIG)                                                             \
    (((REGTRIG) == ADC_EXTERNALTRIG_T1_TRGO2) || ((REGTRIG) == ADC_EXTERNALTRIG_T1_CC4) ||  \
     ((REGTRIG) == ADC_EXTERNALTRIG_T3_TRGO) || ((REGTRIG) == ADC_EXTERNALTRIG_EXT_IT11) || \
     ((REGTRIG) == ADC_SOFTWARE_START))
...
#define IS_ADC_REGULAR_RANK_SEQ_FIXED(RANK) (((RANK) == ADC_RANK_CHANNEL_NUMBER) || ((RANK) == ADC_RANK_NONE))

#define IS_ADC_REGULAR_RANK(RANK)                                                                          \
    (((RANK) == ADC_REGULAR_RANK_1) || ((RANK) == ADC_REGULAR_RANK_2) || ((RANK) == ADC_REGULAR_RANK_3) || \
     ((RANK) == ADC_REGULAR_RANK_4) || ((RANK) == ADC_REGULAR_RANK_5) || ((RANK) == ADC_REGULAR_RANK_6) || \
...
#define IS_ADC_CHANNEL(CHANNEL)                                                                                      \
    (((CHANNEL) == ADC_CHANNEL_0) || ((CHANNEL) == ADC_CHANNEL_1) || ((CHANNEL) == ADC_CHANNEL_2) ||                 \
     ...
     ((CHANNEL) == ADC_CHANNEL_18) || ((CHANNEL) == ADC_CHANNEL_TEMPSENSOR) || ((CHANNEL) == ADC_CHANNEL_VREFINT) || \
     ((CHANNEL) == ADC_CHANNEL_VBAT))

#define IS_ADC_SAMPLING_TIME_COMMON(SAMPLING_TIME_COMMON) \
    (((SAMPLING_TIME_COMMON) == ADC_SAMPLINGTIME_COMMON_1) || ((SAMPLING_TIME_COMMON) == ADC_SAMPLINGTIME_COMMON_2))

#define IS_ADC_SAMPLE_TIME(TIME)                                                       \
    (((TIME) == ADC_SAMPLETIME_1CYCLE_5) || ((TIME) == ADC_SAMPLETIME_3CYCLES_5) ||    \
     ((TIME) == ADC_SAMPLETIME_7CYCLES_5) || ((TIME) == ADC_SAMPLETIME_12CYCLES_5) ||  \
     ((TIME) == ADC_SAMPLETIME_19CYCLES_5) || ((TIME) == ADC_SAMPLETIME_39CYCLES_5) || \
     ((TIME) == ADC_SAMPLETIME_79CYCLES_5) || ((TIME) == ADC_SAMPLETIME_160CYCLES_5))

#define IS_ADC_ANALOG_WATCHDOG_NUMBER(WATCHDOG)                                      \
    (((WATCHDOG) == ADC_ANALOGWATCHDOG_1) || ((WATCHDOG) == ADC_ANALOGWATCHDOG_2) || \
...
#define IS_ADC_EVENT_TYPE(EVENT)                                                                   \
    (((EVENT) == ADC_EOSMP_EVENT) || ((EVENT) == ADC_AWD1_EVENT) || ((EVENT) == ADC_AWD2_EVENT) || \
     ((EVENT) == ADC_AWD3_EVENT) || ((EVENT) == ADC_OVR_EVENT))
