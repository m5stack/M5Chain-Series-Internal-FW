...
/* Private variables ---------------------------------------------------------*/
/* USER CODE BEGIN PV */
extern __IO uint8_t g_uart_in_rx_buf[UART_BUFFER_SIZE][BUFFER_SIZE ];
extern __IO uint8_t g_uart_out_rx_buf[UART_BUFFER_SIZE][BUFFER_SIZE ];
...
static uint8_t s_enum_please_data_packet_buf[9] = { 0xAA, 0x55, 0x03, 0x00,
		0xFF, 0xFC, 0xFB, 0x55, 0xAA };
uint8_t g_heartbeat_data_pack_buf[9] = { 0xAA, 0x55, 0x03, 0x00, 0xFF, 0xFD,
		0xFC, 0x55, 0xAA };
	...
	if (LL_DMA_IsActiveFlag_TC2(DMA1)) {
		...
		if (tx_in_buffer.packet_count > 0) {
			...
			LL_DMA_SetMemoryAddress(DMA1, LL_DMA_CHANNEL_2,
					(uint32_t) tx_in_buffer.send_queue[index].data);
  ...
  /* USER CODE BEGIN DMA1_Ch4_5_DMAMUX1_OVR_IRQn 0 */
	...
	if (LL_DMA_IsActiveFlag_TC4(DMA1)) {
		...
		if (tx_out_buffer.packet_count > 0) {
			...
			LL_DMA_SetMemoryAddress(DMA1, LL_DMA_CHANNEL_4,
					(uint32_t) tx_out_buffer.send_queue[index].data);
	...
	if (LL_DMA_IsActiveFlag_TC5(DMA1)) {
		...
		LL_TIM_DisableCounter(TIM1);       // Disable TIM1 counter, stopping any
										   // timer-based triggers associated with Channel 5
...
{
    uint32_t sr = TIM3->SR;
	TIM3->SR = ~sr;          // 先全清，再处理快照
	/* ═══════════════════════════════════════════
	 * 回调2：CC1 中断（CNT == CCR1 时触发）
	 * 作用：熄灭当前列（亮度截止点）
	 * ═══════════════════════════════════════════ */
    if ((sr & TIM_SR_CC1IF) && (TIM3->DIER & TIM_DIER_CC1IE))
    {
        TIM3->DIER &= ~TIM_DIER_CC1IE;
        chain_mono_all_off();
    }
	/* ═══════════════════════════════════════════
	 * 回调1：Update 中断（CNT 溢出归零时触发）
	 * 作用：切到下一列，点亮 LED
	 * 频率：每 420µs 一次 
	 * ═══════════════════════════════════════════ */
    if ((sr & TIM_SR_UIF) && (TIM3->DIER & TIM_DIER_UIE))
    {
        TIM3->DIER &= ~TIM_DIER_CC1IE;     // 禁用 CC1 中断
        chain_mono_all_off();
        uint8_t g = gamma_table[g_brightness_level];
        if (g == 0) {
            goto next_col;
        }
        uint32_t cathode_moder = 0;
        uint8_t  lit_count     = 0;
        for (uint8_t row = 0; row < 9; row++) {
            uint8_t gray = buf_front[row][s_col] & 0x07;
            if (gray != 0) {
                uint32_t pin = led_refresh_map[row];
                cathode_moder |= (1u << (__builtin_ctz(pin) * 2));
                lit_count++;
            }
        }
        if (lit_count > 0) {
        	uint32_t anode_pin     = led_refresh_map[s_col];
            uint32_t anode_pos     = __builtin_ctz(anode_pin);
            uint32_t anode_moder = (1u << (anode_pos * 2));
            GPIOA->BSRR  = anode_pin;
            GPIOA->MODER |= (anode_moder | cathode_moder);
            uint16_t t_on = (uint16_t)((uint32_t)120 * g * comp_table[lit_count] / 256);
            if (t_on < 1)    t_on = 1;
            if (t_on > 1920) t_on = 1920;
            TIM3->CCR1 = t_on;
  ...
  /* USER CODE BEGIN USART1_IRQn 0 */
	...
	if (LL_USART_IsActiveFlag_IDLE(USART1)
		...
		while (p <= end - MIN_PACKET_SIZE) {
			...
			if (payload_len
					> (BUFFER_SIZE - 6)|| total_length < MIN_PACKET_SIZE) {
				p++;
				continue;
			}
			...
			// check packet tail
			if (p[total_length - 2] != PACK_END_HIGH
					|| p[total_length - 1] != PACK_END_LOW) {
				p++;
				continue;
			}
