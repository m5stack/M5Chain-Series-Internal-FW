...
/* USER CODE BEGIN PV */
// DMA receive buffers
__IO uint8_t g_uart_in_rx_buf[UART_BUFFER_SIZE][BUFFER_SIZE];   // uart_in receive buffer for DMA
__IO uint8_t g_uart_out_rx_buf[UART_BUFFER_SIZE][BUFFER_SIZE];  // uart_out receive buffer for DMA
...
void chain_init(void)
{
    ...
    // Check if the screen rotation setting is set to maximum (0xFF)
    if (get_screen_rotation() == 0xFF) {
        // Set the rotation to a base value if it is maximum
        g_current_rotation = SCREEN_ROTATION_BASE;
        set_screen_rotation((uint8_t)g_current_rotation);
    } else {
        // Otherwise, get the current screen rotation setting
        g_current_rotation = get_screen_rotation();
    }

    // Check if the brightness level setting is set to maximum (0xFF)
    if (get_brightness_level() == 0xFF) {
        // Set the brightness to a base level if it is maximum
        g_brightness_level = RRIGTNESS_LEVEL_BASE;
        set_brightness_level((uint8_t)g_brightness_level);
    } else {
        // Otherwise, get the current brightness level setting
        chain_mono_set_brightness(get_brightness_level());
    }
}
...
int main(void)
{
    ...
    /* USER CODE BEGIN 2 */
    HAL_Delay(1);
    HAL_TIM_Base_Start_IT(&htim3);               // Update 中断
    HAL_TIM_OC_Start_IT(&htim3, TIM_CHANNEL_1);  // CC1 中断
    HAL_Delay(1);
    ...
    while (1) {
        ...
        chain_mono_scroll_text_update(tick_ms);
        if (g_cmd_status == CMD_SPACE_BUSY_STATUS) {
            switch (g_cmd_buf[0]) {
                case CHAIN_MONO_SET_MODE:
                    chain_mono_set_mode_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_MONO_GET_MODE:
                    chain_mono_get_mode_handle();
                    break;
                case CHAIN_MONO_SET_PIXEL:
                    chain_mono_set_pixel_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_MONO_GET_PIXEL:
                    chain_mono_get_pixel_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_MONO_SET_DISPLAY_BUFFER:
                    chain_mono_set_display_buffer_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_MONO_GET_DISPLAY_BUFFER:
                    chain_mono_get_display_buffer_handle();
                    break;
                case CHAIN_MONO_SET_ROTATION:
                    chain_mono_set_rotation_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_MONO_GET_ROTATION:
                    chain_mono_get_rotation_handle();
                    break;
                case CHAIN_MONO_SET_BRIGHTNESS:
                    chain_mono_set_brightness_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_MONO_GET_BRIGHTNESS:
                    chain_mono_get_brightness_handle();
                    break;
                case CHAIN_MONO_SET_SCROLL_TEXT:
                    chain_mono_set_scroll_text_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_MONO_GET_SCROLL_TEXT:
                    chain_mono_get_scroll_text_handle();
                    break;
                case CHAIN_MONO_SET_SCROLL_TEXT_STATE:
                    chain_mono_set_scroll_text_state_handle(g_cmd_buf[1]);
                    break;
                case CHAIN_MONO_GET_SCROLL_TEXT_STATE:
                    chain_mono_get_scroll_text_state_handle();
                    break;
                case CHAIN_MONO_SET_CHAR:
                    chain_mono_set_char_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_MONO_CLEAR:
                    chain_mono_clear_handle();
                    break;
                case CHAIN_GET_UID:
