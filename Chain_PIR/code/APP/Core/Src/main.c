...
__IO uint8_t g_rgb_set_complete_flag = 0;  // Status indicating if the rgb dma send data to port finished
                                           // (0: sending, 1: send complete)

// Device function variables
__IO uint8_t g_trigger_keep_seconds = 0;
...
void chain_init(void)
{
    ...
    if ((get_trigger_keep_seconds() == 0xFF) && (get_is_trigger_keep_time_set() == 0xFF)) {
        g_trigger_keep_seconds = TRIGGER_KEEP_SECONDS_DEAULT;

        set_trigger_keep_seconds(g_trigger_keep_seconds);
    } else {
        g_trigger_keep_seconds = get_trigger_keep_seconds();
    }
}
        ...
        if (g_cmd_status == CMD_SPACE_BUSY_STATUS) {
            switch (g_cmd_buf[0]) {
                ...
                case CHAIN_GET_RGB_LIGHT:
                    ...
                    break;
                case CHAIN_PIR_GET_IR:
                    chain_pir_get_ir_induction();
                    break;
                case CHAIN_PIR_SET_AUTO_SEND_IR_STATUS:
                    chain_pir_set_auto_send_status(g_cmd_buf[1]);
                    break;
                case CHAIN_PIR_GET_AUTO_SEND_IR_STATUS:
                    chain_pir_get_auto_send_status();
                    break;
                case CHAIN_SET_TRIGGER_DELAY:
                    chain_pir_set_trigger_keep_seconds(g_cmd_buf[1], g_cmd_buf[2]);
                    break;
                case CHAIN_GET_TRIGGER_DELAY:
                    chain_pir_get_trigger_delay_times();
                    break;
