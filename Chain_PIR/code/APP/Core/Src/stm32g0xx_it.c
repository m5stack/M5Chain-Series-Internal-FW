...
void USART1_IRQHandler(void)
{
    ...
    if (LL_USART_IsActiveFlag_IDLE(USART1) && LL_USART_IsEnabledIT_IDLE(USART1)) {
        ...
        while (p <= end - MIN_PACKET_SIZE) {
            // check header
            if (p[0] != PKT_HEADER1 || p[1] != PKT_HEADER2) {
                p++;
                continue;
            }
            ...
            // check packet tail
            if (p[total_length - 2] != PKT_TAIL1 || p[total_length - 1] != PKT_TAIL2) {
                p++;
                continue;
            }
...
void USART2_IRQHandler(void)
{
    ...
    if (LL_USART_IsActiveFlag_IDLE(USART2) && LL_USART_IsEnabledIT_IDLE(USART2)) {
        ...
        while (p <= end - MIN_PACKET_SIZE) {
            ...
            if (p[total_length - 2] != 0x55 || p[total_length - 1] != 0xAA) {
                p++;
                continue;
            }
