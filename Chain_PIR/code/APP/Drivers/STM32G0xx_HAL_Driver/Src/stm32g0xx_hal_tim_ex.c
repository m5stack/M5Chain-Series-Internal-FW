    ...
    switch (BreakInput) {
        case TIM_BREAKINPUT_BRK: {
            ...
            if (READ_BIT(htim->Instance->BDTR, TIM_BDTR_BKBID) == TIM_BDTR_BKBID) {
                /* Break input BRK is re-armed automatically by hardware. Poll to check whether fault condition
                 * disappeared */
        ...
        case TIM_BREAKINPUT_BRK2: {
            ...
            if (READ_BIT(htim->Instance->BDTR, TIM_BDTR_BK2BID) == TIM_BDTR_BK2BID) {
                /* Break input BRK2 is re-armed automatically by hardware. Poll to check whether fault condition
                 * disappeared */
                /* Init tickstart for timeout management */
                tickstart = HAL_GetTick();
