...
static void rgb_setcolor(uint8_t led_id, RGB_Color_TypeDef color)
{
    ...
    if (fade_flag) {
        color.R = sat_add(color.R, event_overlay.R);
        color.G = sat_add(color.G, event_overlay.G);
        color.B = sat_add(color.B, event_overlay.B);
    }
...
///**
// * @brief Set RGB values based on the provided color.
// * @param rgb_index index of RGB number.
// * @param color used to set RGB color.
// * @retval None
// */
// void send_rgb_value_to_switch(uint8_t rgb_index, RGB_Color_TypeDef color) {
//
//	 // Set the RGB color to the first LED
//	rgb_setcolor(rgb_index, color);
//
 ...
 * @param size The size of the RGB data (should be 3 for R, G, B(old version) or 5 for Index, Num, R, G, B).
 * @retval None
 */
void chain_set_rgb_value(uint8_t *buffer, uint16_t size)
{
    uint8_t operation_result = RGB_OPERATION_FAIL;

    // Validate input and check buffer format
    if (buffer != NULL && size == 5 && buffer[0] == RGB_INDEX && buffer[1] == RGB_NUM) {
        color.R = buffer[2];
        color.G = buffer[3];
        color.B = buffer[4];
        //		USER_COLOR.R = buffer[2];
        //		USER_COLOR.G = buffer[3];
        //		USER_COLOR.B = buffer[4];

        s_rgb_update_flag = 1;

        //		for (int i = 0; i < RGB_NUM; i++) {
        //			rgb_setcolor(RGB_INDEX + i, color);
        //		}
        //		// Reset the load to prepare for sending new data
        //		reset_load();
        //
        ...
        operation_result = RGB_OPERATION_SUCCESS;
    }
 ...
 * @retval size The size of the command buffer (should be 2)
 */
void chain_get_rgb_value(uint8_t *buffer, uint16_t size)
{
    ...
    if (operation_result == RGB_OPERATION_SUCCESS) {
...
void rgb_update(void)
{
    ...
    if (fade_frames_left > 0 && (now - last_step_tick) >= RGB_FADE_MS_STEP) {
        last_step_tick = now;
        if (fade_type == 1) {  // R
            event_overlay.R = (uint8_t)(((fade_frames_left - 1) * RGB_FADE_MAX) / (RGB_FADE_FRAMES - 1));
            event_overlay.G = 0;
            event_overlay.B = 0;
        } else if (fade_type == 2) {  // G
            event_overlay.G = (uint8_t)(((fade_frames_left - 1) * RGB_FADE_MAX) / (RGB_FADE_FRAMES - 1));
            event_overlay.R = 0;
            event_overlay.B = 0;
        }
        fade_frames_left--;
