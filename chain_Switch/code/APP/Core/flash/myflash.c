...
/**
 * @brief Modify the slip change mode.
 * @note This function updates the slip change mode stored in flash memory
 *       with the provided value.
 *
 * @param data The value to modify the slip change mode.
 * @retval true if the modification is successful, false otherwise.
 */
bool set_slip_mode_status(uint8_t data)
{
    __disable_irq();
    uint64_t temp =
        my_flash_read_double_word(STM32G0xx_FLASH_PAGE31_STARTADDR);  // Read the current double word from flash memory
    set_byte_in_uint64(&temp, 0, data);  // Set the specified byte in the double word to the new data
    my_flash_earse_pages(STM32G0xx_FLASH_PAGE31_STARTADDR);  // Erase the flash page before writing

    // Continuously attempt to write the modified double word to flash until successful
    while (my_flash_write_double_word(STM32G0xx_FLASH_PAGE31_STARTADDR, temp) != true) {
    }
    __enable_irq();
    return (get_slip_mode_status() == data);  // Return false if the data does not match
}

/**
 * @brief Get the current slip change mode.
 * @note This function reads the slip change mode stored at the defined address.
 *
 * @param None
 * @retval The current clockwise status value (0 or 1).
 */
uint8_t get_slip_mode_status(void)
{
    return *((__IO uint8_t *)(SLIP_STATUS_ADDR));  // Read and return the value from the specified address
}

/**
 * @brief Get the current slip threshold value.
 * @note This function reads the slip threshold value stored at the defined address.
 *
 * @param None
 * @retval The current clockwise status value (0 or 1).
 */
...
{
    return *((__IO uint32_t *)(SLIP_THRESHOLD_ADDR));  // Read and return the value from the specified address
}

/**
 * @brief Modify the slip open&close threshold.
 * @note This function updates the slip open&close threshold stored in flash memory
 *       with the provided value.
 *
 * @param data The value to modify the slip threshold.
 * @retval true if the modification is successful, false otherwise.
 */
                          ...
                          uint8_t close_low_values, uint8_t close_high_values)
{
    ...
    set_byte_in_uint64(&temp, 5, open_low_values);    // Modify the brightness byte
    set_byte_in_uint64(&temp, 4, open_high_values);   // Modify the brightness byte
    set_byte_in_uint64(&temp, 3, close_low_values);   // Modify the brightness byte
    set_byte_in_uint64(&temp, 2, close_high_values);  // Modify the brightness byte
    ...
    __enable_irq();
    return (get_slip_threshold() == total_threshold);  // Return false if the data does not match
}
