...
#define IS_LL_ADC_CLOCK_FREQ_MODE(__CLOCK_FREQ_MODE__) \
    (((__CLOCK_FREQ_MODE__) == LL_ADC_CLOCK_FREQ_MODE_HIGH) || ((__CLOCK_FREQ_MODE__) == LL_ADC_CLOCK_FREQ_MODE_LOW))

/* Check of parameters for configuration of ADC hierarchical scope:           */
/* ADC instance.                                                              */
#define IS_LL_ADC_CLOCK(__CLOCK__)                                                                   \
    (((__CLOCK__) == LL_ADC_CLOCK_SYNC_PCLK_DIV4) || ((__CLOCK__) == LL_ADC_CLOCK_SYNC_PCLK_DIV2) || \
...
#define IS_LL_ADC_RESOLUTION(__RESOLUTION__)                                                       \
    (((__RESOLUTION__) == LL_ADC_RESOLUTION_12B) || ((__RESOLUTION__) == LL_ADC_RESOLUTION_10B) || \
     ((__RESOLUTION__) == LL_ADC_RESOLUTION_8B) || ((__RESOLUTION__) == LL_ADC_RESOLUTION_6B))
...
#define IS_LL_ADC_LOW_POWER(__LOW_POWER__)                                                  \
    (((__LOW_POWER__) == LL_ADC_LP_MODE_NONE) || ((__LOW_POWER__) == LL_ADC_LP_AUTOWAIT) || \
     ((__LOW_POWER__) == LL_ADC_LP_AUTOPOWEROFF) || ((__LOW_POWER__) == LL_ADC_LP_AUTOWAIT_AUTOPOWEROFF))
...
#define IS_LL_ADC_REG_SEQ_MODE(__REG_SEQ_MODE__) \
    (((__REG_SEQ_MODE__) == LL_ADC_REG_SEQ_FIXED) || ((__REG_SEQ_MODE__) == LL_ADC_REG_SEQ_CONFIGURABLE))
...
ErrorStatus LL_ADC_DeInit(ADC_TypeDef *ADCx)
{
    ...
    if (READ_BIT(ADCx->CR, (ADC_CR_ADSTP | ADC_CR_ADSTART | ADC_CR_ADDIS | ADC_CR_ADEN)) == 0UL) {
        /* ========== Reset ADC registers ========== */
        /* Reset register IER */
        CLEAR_BIT(ADCx->IER, (LL_ADC_IT_ADRDY | LL_ADC_IT_EOC | LL_ADC_IT_EOS | LL_ADC_IT_OVR | LL_ADC_IT_EOSMP |
                              LL_ADC_IT_AWD1 | LL_ADC_IT_AWD2 | LL_ADC_IT_AWD3 | LL_ADC_IT_EOCAL | LL_ADC_IT_CCRDY));

        /* Reset register ISR */
        SET_BIT(ADCx->ISR,
                (LL_ADC_FLAG_ADRDY | LL_ADC_FLAG_EOC | LL_ADC_FLAG_EOS | LL_ADC_FLAG_OVR | LL_ADC_FLAG_EOSMP |
                 LL_ADC_FLAG_AWD1 | LL_ADC_FLAG_AWD2 | LL_ADC_FLAG_AWD3 | LL_ADC_FLAG_EOCAL | LL_ADC_FLAG_CCRDY));
        ...
        CLEAR_BIT(ADCx->CFGR1, (ADC_CFGR1_AWD1CH | ADC_CFGR1_AWD1EN | ADC_CFGR1_AWD1SGL | ADC_CFGR1_DISCEN |
                                ADC_CFGR1_CHSELRMOD | ADC_CFGR1_AUTOFF | ADC_CFGR1_WAIT | ADC_CFGR1_CONT |
                                ADC_CFGR1_OVRMOD | ADC_CFGR1_EXTEN | ADC_CFGR1_EXTSEL | ADC_CFGR1_ALIGN |
                                ADC_CFGR1_RES | ADC_CFGR1_SCANDIR | ADC_CFGR1_DMACFG | ADC_CFGR1_DMAEN));

        /* Reset register SMPR */
        CLEAR_BIT(ADCx->SMPR, ADC_SMPR_SMP1 | ADC_SMPR_SMP2 | ADC_SMPR_SMPSEL);

        /* Reset register CHSELR */
        CLEAR_BIT(ADCx->CHSELR, (ADC_CHSELR_CHSEL18 | ADC_CHSELR_CHSEL17 | ADC_CHSELR_CHSEL16 | ADC_CHSELR_CHSEL15 |
                                 ADC_CHSELR_CHSEL14 | ADC_CHSELR_CHSEL13 | ADC_CHSELR_CHSEL12 | ADC_CHSELR_CHSEL11 |
                                 ADC_CHSELR_CHSEL10 | ADC_CHSELR_CHSEL9 | ADC_CHSELR_CHSEL8 | ADC_CHSELR_CHSEL7 |
                                 ADC_CHSELR_CHSEL6 | ADC_CHSELR_CHSEL5 | ADC_CHSELR_CHSEL4 | ADC_CHSELR_CHSEL3 |
        ...
        /*       already done above.                                              */
        CLEAR_BIT(ADCx->CFGR2, (ADC_CFGR2_CKMODE | ADC_CFGR2_TOVS | ADC_CFGR2_OVSS | ADC_CFGR2_OVSR | ADC_CFGR2_OVSE));
...
ErrorStatus LL_ADC_Init(ADC_TypeDef *ADCx, const LL_ADC_InitTypeDef *pADC_InitStruct)
{
    ...
    if (LL_ADC_IsEnabled(ADCx) == 0UL) {
        ...
        MODIFY_REG(ADCx->CFGR1, ADC_CFGR1_RES | ADC_CFGR1_ALIGN | ADC_CFGR1_WAIT | ADC_CFGR1_AUTOFF,
                   pADC_InitStruct->Resolution | pADC_InitStruct->DataAlignment | pADC_InitStruct->LowPowerMode);
    ...
    if (LL_ADC_IsEnabled(ADCx) == 0UL) {
        ...
        if ((LL_ADC_REG_GetSequencerConfigurable(ADCx) == LL_ADC_REG_SEQ_FIXED) ||
            ...
            MODIFY_REG(ADCx->CFGR1,
                       ADC_CFGR1_EXTSEL | ADC_CFGR1_EXTEN | ADC_CFGR1_DISCEN | ADC_CFGR1_CONT | ADC_CFGR1_DMAEN |
                           ADC_CFGR1_DMACFG | ADC_CFGR1_OVRMOD,
                       pADC_RegInitStruct->TriggerSource | pADC_RegInitStruct->SequencerDiscont |
                           pADC_RegInitStruct->ContinuousMode | pADC_RegInitStruct->DMATransfer |
