...
// RGB test states for state machine
...
uint8_t g_delay_flag = 0;
uint32_t g_delay     = 0;
uint8_t g_next_step  = 0;
uint8_t now_step     = 1;
...
void chain_init(void)
{
    ...
    if (get_screen_rotation() == 0xFF) {
        ...
        g_current_rotation = RGB_ROTATION_0;
    ...
    if (get_brightness_level() == 0xFF) {
        ...
        set_brightness_level(g_light);
    } else {
        // Otherwise, get the current RGB light setting
        g_light = get_brightness_level();
    }
}
...
int main(void)
{
    ...
    while (1) {
        if (g_cmd_status == CMD_SPACE_BUSY_STATUS) {
            switch (g_cmd_buf[0]) {
                case CHAIN_RGB_SET_MODE:
                    chain_rgb_set_mode_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_RGB_GET_MODE:
                    chain_rgb_get_mode_handle();
                    break;
                case CHAIN_RGB_SET_PIXEL:
                    chain_rgb_set_pixel_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_RGB_GET_PIXEL:
                    chain_rgb_get_pixel_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_RGB_SET_DISPLAY_BUFFER:
                    chain_rgb_set_display_buffer_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_RGB_GET_DISPLAY_BUFFER:
                    chain_rgb_get_display_buffer_handle();
                    break;
                case CHAIN_RGB_SET_ROTATION:
                    chain_rgb_set_rotation_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_RGB_GET_ROTATION:
                    chain_rgb_get_rotation_handle();
                    break;
                case CHAIN_RGB_SET_BRIGHTNESS:
                    chain_rgb_set_brightness_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_RGB_GET_BRIGHTNESS:
                    chain_rgb_get_brightness_handle();
                    break;
                case CHAIN_RGB_SET_SCROLL_TEXT:
                    chain_rgb_set_scroll_text_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_RGB_GET_SCROLL_TEXT:
                    chain_rgb_get_scroll_text_handle();
                    break;
                case CHAIN_RGB_SET_SCROLL_TEXT_STATE:
                    chain_rgb_set_scroll_text_state_handle(g_cmd_buf[1]);
                    break;
                case CHAIN_RGB_GET_SCROLL_TEXT_STATE:
                    chain_rgb_get_scroll_text_state_handle();
                    break;
                case CHAIN_RGB_SET_CHAR:
                    chain_rgb_set_char_handle((uint8_t *)(g_cmd_buf + 1), (g_cmd_size - 1));
                    break;
                case CHAIN_RGB_CLEAR:
                    chain_rgb_clear_handle();
                    break;
                case CHAIN_GET_UID:
        ...
        /* USER CODE BEGIN 3 */
        chain_rgb_scroll_text_update(HAL_GetTick());
        chain_rgb_update();
