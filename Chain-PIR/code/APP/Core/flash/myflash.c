...
/**
 * @brief Get the number of seconds to keep the trigger active.
 * @note This function reads the trigger keep time stored at the
 *       defined address.
 *
 * @param None
 * @retval The number of seconds to keep the trigger active.
 */
uint8_t get_trigger_keep_seconds(void)
{
    return *((__IO uint8_t *)(TRIGGER_KEEPTIME_ADDR));  // Read and return the trigger keep time
}

/**
 * @brief Get the status of whether the trigger keep time has been set.
 * @note This function reads the status from the defined address to check if the trigger keep time has been set.
 *
 * @param None
 * @retval The status of whether the trigger keep time has been set.
 */
uint8_t get_is_trigger_keep_time_set(void)
{
    return *((__IO uint8_t *)(IS_TRIGGER_KEEPTIME_SET_ADDR));  // Read and return the trigger keep time status
}

/**
 * @brief Set the number of seconds to keep the trigger active.
 * @note This function modifies the trigger keep time stored at the
 *       defined address.
 *
 * @param seconds The number of seconds to keep the trigger active.
 * @retval true if the trigger keep time is successfully set, false otherwise.
 */
bool set_trigger_keep_seconds(uint8_t seconds)
{
    __disable_irq();
    uint64_t temp = my_flash_read_double_word(STM32G0xx_FLASH_PAGE31_STARTADDR);

    set_byte_in_uint64(&temp, 2, seconds);  // Modify the trigger keep time byte
    set_byte_in_uint64(&temp, 3, 1);        // Set the status byte to indicate that the trigger keep time is set
    ...
    __enable_irq();
    return (get_trigger_keep_seconds() == seconds);  // Verify if the new trigger keep time has been successfully set
}
