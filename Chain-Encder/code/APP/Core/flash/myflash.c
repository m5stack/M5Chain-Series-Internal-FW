...
/**
 * @brief  Modify the AB status
 * @note   This function modifies the AB status by writing the specified
 *         data to a predefined flash memory address. It verifies the
 *         write operation by reading back the value.
 *
 * @param  data The new AB status value to be set (should be 0 or 1).
...
bool set_ab_status(uint8_t data)
{
    ...
    // Set the specified byte in the 64-bit variable
    set_byte_in_uint64(&temp, 0, data);

    // Erase the specified flash page before writing
    my_flash_earse_pages(STM32G0xx_FLASH_PAGE31_STARTADDR);

    // Write the modified value to flash until successful
    while (my_flash_write_double_word(STM32G0xx_FLASH_PAGE31_STARTADDR, temp) != true) {
    }
    __enable_irq();

    return (data == get_ab_status());  // Operation failed
}

/**
 * @brief  Retrieve the AB status
 * @note   This function reads the current AB status from the specified
 *         memory address.
 *
 * @param  None
